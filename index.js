...
function sum(numbers) {
  let count = 0;
  numbers.filter((item) => (item % 2 !== 0 ? (count += item) : false));
  return count;
}
...
function hasEqualSums(first, second) {
  const arr1 = first.reduce((sum, num) => sum + num, 0);
  const arr2 = second.reduce((sum, num) => sum + num, 0);
...
function filterNames(names, number) {
  return names.filter((item) => item.length >= number);
}
